...
__global__ void reduce(const float* input, int N) {
...
__global__ void reduce_final(float* output, unsigned int nb) {
    __shared__ float sum_per_warp[32];
    int tid = threadIdx.x;
    int warp_id = tid / WARP_SIZE;
...
void solve(const float* input, float* output, int N) {
    ...
    reduce<<<nb, THREADS_PER_BLOCK>>>(input, N);
    reduce_final<<<1, 1024>>>(output, nb);
}
